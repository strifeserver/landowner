...
class BaseModel:
    ...
    def index(
        ...
        for record in data:
            ...
            if search:
                if any(
                    search.lower() in str(value).lower() for value in record.values()
                ):
            ...
            if filters:
                ...
                for key, value in filters.items():
                    match key:
                        case "id":
                            try:
                                match = match and int(record.get("id", -1)) == int(
                                    value
                                )
                            except ValueError:
                                match = False
                        ...
                        case "created_at_from":
                            try:
                                record_date = datetime.strptime(
                                    record.get("created_at", ""), "%Y-%m-%d %H:%M:%S"
                                )
