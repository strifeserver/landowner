...
def setup_date_filters(self, window):
    ...
    tk.Checkbutton(window, text="Date Filters", variable=self.enable_date_filters).pack(
        pady=(10, 5)
    )
    ...
    def create_date_widgets():
        ...
        self.date_created_entry = DateEntry(
            self.dc_frame, date_pattern="yyyy-mm-dd", width=16
        )
        ...
        self.date_updated_entry = DateEntry(
            self.du_frame, date_pattern="yyyy-mm-dd", width=16
        )
    ...
    # Toggle view and create on demand
    def toggle_date_filters(*_):
        if self.enable_date_filters.get():
            create_date_widgets()  # Lazy creation
