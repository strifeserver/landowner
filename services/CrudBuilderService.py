...
class CrudBuilderService(BaseService):
    def __init__(self):
        ...
        # Templates are in ../crud_builder/templates relative to this file
        template_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'crud_builder', 'templates')
    ...
    def generate_module(self, data):
        ...
        if getattr(sys, 'frozen', False):
            return {"success": False, "message": "CRUD Builder is disabled in the compiled application."}
    ...
    def _generate_controller(self, name):
        ...
        content = template.render(class_name=class_name, name=name)
    ...
    def _generate_migration(self, name, table_name, fields_json):
        ...
        template = self.env.get_template("migration.py.j2")
        content = template.render(
            table_name=table_name,
            cols_str=cols_str,
